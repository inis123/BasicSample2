...
public interface PersonDao {
    ...
    @Query("SELECT * FROM person p WHERE p.personID=:personId")
    PersonEntity getById(int personId);

    @Query("SELECT * FROM person p WHERE EXISTS(SELECT * FROM interessent i WHERE i.personID=personID )")
    List<PersonEntity> getAllWithI();
